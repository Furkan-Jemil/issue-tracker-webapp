...
export async function getAdminExport(c: Context) {
  try {
    ...
    const rate = checkRateLimit('admin-export:get', session.user.id, 5, 60_000)
    if (rate) return c.json(rate.body, ({ status: rate.status } as any))

    const batchSize = parseBatchSize(c.req.url)
    ...
    const [usersCount, issuesCount, commentsCount, screenshotsCount, notificationsCount, historyCount] = counts
