...
/**
 * VERBATIM port of apps/web/server/routes/upload.ts.
 *
 * Constants are copied inline from apps/web/src/lib/issueValidation.ts to keep
 * apps/api standalone. Both handlers return a Web `Response` so the controller
 * bridges them back to Express unchanged — preserving every body, status, and
 * the multipart / base64 semantics exactly.
 */
...
export class UploadService {
  ...
  async handleMultipart(req: Request): Promise<Response> {
    try {
      const session = await this.sessions.getServerSession(req.headers);
      ...
      if (files.length > MAX_FILE_COUNT) {
        return new Response(
          JSON.stringify({ error: `Maximum ${MAX_FILE_COUNT} files allowed` }),
      ...
      if (
        ...
        return new Response(
          JSON.stringify({ error: 'No valid files uploaded', rejected: rejectedFiles }),
          { status: 400, headers: { 'content-type': 'application/json' } },
        );
      }
      ...
      if (
        ...
        return new Response(
          JSON.stringify({ error: 'No valid attachments uploaded', attachmentsRejected: rejectedAttachments }),
          { status: 400, headers: { 'content-type': 'application/json' } },
        );
      }
  ...
  // POST /api/upload/base64 — mobile base64 JSON payload.
  async handleBase64(req: Request): Promise<Response> {
    try {
      const session = await this.sessions.getServerSession(req.headers);
      if (!session?.user) {
        return new Response(JSON.stringify({ error: 'Unauthorized' }), {
      ...
      const files: { name: string; type: string; content: string; sizeBytes: number; kind: string }[] =
        body?.files ?? [];
      ...
      return new Response(JSON.stringify({ screenshots, attachments }), {
        headers: { 'content-type': 'application/json' },
      });
    } catch (error) {
